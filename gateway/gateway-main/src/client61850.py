...
class iec61850client():
	...
	def printValue(value):
		...
		if _type == "float":
			#MmsValue_toFloat = Get the float value of a MmsValue object
		...
		if deviceList:
			...
			while device:
				...
				while logicalNode:
					...
					#iec61850.IedConnection_getLogicalNodeVariables(con, LD_name+"/"+LN_name)
					...
					while LNobject:
						...
						#LinkedList_getNext = Get the next element in the list
						LNobject = lib61850.LinkedList_getNext(LNobject)
					#Delete a LinkedList object
					lib61850.LinkedList_destroy(LNobjects)
					#IedConnection_getLogicalNodeDirectory = returns the directory of the given logical node (LN) containing elements of the specified ACSI class
					...
					#returns the directory of the given logical node (LN)
					# containing elements of the specified ACSI class
	...
	def writeValue(con, model, ref, value):
		...
		error = lib61850.IedClientError()
		#write a functional constrained data attribute
			...
			if model: #if model is not empty
				...
				if tupl in self.reporting and len(self.reporting[tupl]) > 0:
					for refdata in self.reporting[tupl]:
						rcb = refdata["rcb"]
						error = lib61850.IedClientError()
						#IedConnection_getRCBValues = Read access to attributes of a report control block (RCB) at the connected server
						...
						#IedConnection_installReportHandler =  	Install a report handler function for the specified
		...
		if err == 0:
			...
			if error == 0:
				...
				if error == 3: #we lost the connection
					#IedConnection_destroy = destroy an IedConnection instance
					lib61850.IedConnection_destroy(con)
					self.connections[tupl]['con'] = None
				return error
		else:
	...
	def ReportHandler_cb(self, param, report):
		...
		#ClientReport_getReasonForInclusion = get the reason code (reason for inclusion) for a specific report data set element
		...
		#ClientReport_getDataSetValues = return the received data set values of the report
	...
	def getDatamodel(self, ref=None, hostname="localhost", port=5102):
		...
		# if port is explicitly defined as "" or None, assume 5102
		if port == "" or port == None:
			port = 5102
	...
	def get_controlObject(self, tupl, uri_ref):
		...
		if not uri_ref.path[1:] in self.connections[tupl]['control'] or self.connections[tupl]['control'][ uri_ref.path[1:] ] == None:
			#ControlObjectClient_create = Create a new client control object
			control = lib61850.ControlObjectClient_create(uri_ref.path[1:], con)

			self.connections[tupl]['control'][ uri_ref.path[1:] ] = control
			#ControlObjectClient_getControlModel = Get the current control model (local representation) applied to the control object
			...
			if ctlModel == lib61850.CONTROL_MODEL_DIRECT_ENHANCED or ctlModel == lib61850.CONTROL_MODEL_SBO_ENHANCED:
				...
				#CommandTerminationHandler = A callback handler that is invoked when a command termination message is received
				cbh = lib61850.CommandTerminationHandler(self.commandTerminationHandler_cb)

				ref = bytes(uri_ref.path[1:].encode('utf-8'))
				#ControlObjectClient_setCommandTerminationHandler = Set the command termination callback handler for this control object
	...
	def operate(self, ref, value):
		...
		if err == 0:
			...
			#ControlObjectClient_setOrigin = Set the origin parameter for control commands
			lib61850.ControlObjectClient_setOrigin(control, "mmi", 3)
			#ControlObjectClient_getCtlValType = Get the type of ctlVal
			mmsType = lib61850.ControlObjectClient_getCtlValType(control)
			ctlVal = iec61850client.getMMsValue("",value,0,mmsType)
			#ControlObjectClient_operate = Send an operate command to the server
	...
	def select(self, ref, value):
		...
		if err == 0:
			...
			if ctlModel == lib61850.CONTROL_MODEL_SBO_NORMAL:
				...
				#ControlObjectClient_select = Send a select command to the server.
				...
				if error == 1:
					...
					#ControlObjectClient_getLastApplError = Get the last received control application error. 
					lastApplError = lib61850.ControlObjectClient_getLastApplError(control)
					addCause = AddCause(lastApplError.addCause).name
				...
				lib61850.ControlObjectClient_setOrigin(control, "mmi", 3)
				#ControlObjectClient_selectWithValue = Send an select with value command to the server
	...
	def cancel(self, ref):
		...
		if err == 0:
			tupl =  hostname + ":" + str(port)
			control = self.get_controlObject(tupl, uri_ref)
			#ControlObjectClient_cancel = Send a cancel command to the server
...
if __name__=="__main__":
	...
	err = cl.registerWriteValue("iec61850://127.0.0.1:10102/IED4_SMVMUnn/MMXU1.AvAPhs.mag.f")