...
def write_value(id, value):
  ...
  if retValue > 0:
    return retValue, client61850.IedClientError(retValue).name
  if retValue == 0:
...
if __name__ == '__main__':
  ...
  logger.info("started")
  client = client61850.iec61850client(readvaluecallback, logger, cmdTerm_cb, Rpt_cb)
  iec104_server = server60870.IEC60870_5_104_server()
  iec104_server.start()
  ...
  if 'measuredvaluescaled' in config:
    for item in config['measuredvaluescaled']:
      ...
      if iec104_server.addioa(int(item), lib60870.MeasuredValueScaled,0,read_60870_callback,True) == 0:
  ...
  if 'singlepointinformation' in config:
    for item in config['singlepointinformation']:
      #create 104 data for GI
      if iec104_server.addioa(int(item), lib60870.SinglePointInformation,0,read_60870_callback,True) == 0:
  ...
  if 'doublepointinformation' in config:
    for item in config['doublepointinformation']:
      #create 104 data for GI
      if iec104_server.addioa(int(item), lib60870.DoublePointInformation,0,read_60870_callback,True) == 0:
  ...
  if 'singlepointcommand' in config:
    for item in config['singlepointcommand']:
      #create 104 data for GI
      if iec104_server.addioa(int(item), lib60870.SingleCommand,0,command_60870_callback,False) == 0:
  ...
  if 'doublepointcommand' in config:
    for item in config['doublepointcommand']:
      #create 104 data for GI
      if iec104_server.addioa(int(item), lib60870.DoubleCommand,0,command_60870_callback,False) == 0:
